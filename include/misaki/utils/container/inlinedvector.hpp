...
namespace misaki::container {

template <typename T, int N, class Allocator = mstd::pmr::polymorphic_allocator<T>>
class InlinedVector {
 public:
  using value_type = T;
  ...
  InlinedVector &operator=(InlinedVector &&other) {
    ...
    clear();
    if (alloc == other.alloc) {
      mstd::swap(ptr, other.ptr);
      mstd::swap(nAlloc, other.nAlloc);
      mstd::swap(nStored, other.nStored);
