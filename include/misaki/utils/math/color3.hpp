...
class TColor3 {
 public:
  ...
  MSK_CPU_GPU constexpr TColor3() noexcept : r(0), g(0), b(0) {}

  MSK_CPU_GPU constexpr TColor3(Value r, Value g, Value b) noexcept : r(r), g(g), b(b) {}

  MSK_CPU_GPU constexpr TColor3(Value r) noexcept : r(r), g(r), b(r) {}

  MSK_CPU_GPU constexpr TColor3(const TVector3<Value> &vec) noexcept : r(vec.x), g(vec.y), b(vec.z) {}

  template <typename T, std::enable_if_t<std::is_arithmetic_v<T>, int> = 0>
  MSK_CPU_GPU constexpr TColor3(const TColor3<T> &col) noexcept : r(Value(col.r)), g(Value(col.g)), b(Value(col.b)) {}
  ...
  MSK_CPU_GPU auto luminance() const {
    return r * 0.212671f + g * 0.715160f + b * 0.072169f;
  }

  MSK_CPU_GPU Self to_srgb() const {
    Self result;
    for (int i = 0; i < 3; ++i) {
      Value value = this->[i];
      if (value <= 0.0031308f)
  ...
  MSK_CPU_GPU Self to_linear() const {
    Self result;
    for (int i = 0; i < 3; ++i) {
      Value value = this->[i];
      if (value <= 0.04045f)
  ...
  MSK_CPU_GPU bool is_black() const {
    return r == 0 || g == 0 || b == 0;
  }
...
// Unary operation
template <typename Value>
MSK_CPU_GPU TColor3<Value> operator-(const TColor3<Value> &col) noexcept {
  return TColor3<Value>(-col.r, -col.g, -col.b);
}

// Binary operation
template <typename Value>
MSK_CPU_GPU TColor3<Value> operator+(const TColor3<Value> &lhs, const TColor3<Value> &rhs) noexcept {
...
MSK_CPU_GPU TColor3<Value> operator+(const TColor3<Value> &lhs, T rhs) noexcept {
