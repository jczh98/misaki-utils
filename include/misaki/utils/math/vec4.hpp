...
class TVector4 {
 public:
  ...
  MSK_XPU constexpr TVector4() noexcept : x(0), y(0), z(0), w(0) {}

  MSK_XPU constexpr TVector4(Value x, Value y, Value z, Value w) noexcept : x(x), y(y), z(z), w(w) {}

  MSK_XPU constexpr TVector4(Value x) noexcept : x(x), y(x), z(x), w(x) {}

  template <typename T, std::enable_if_t<std::is_arithmetic_v<T>, int> = 0>
  MSK_XPU constexpr TVector4(const TVector4<T> &vec) noexcept : x(Value(vec.x)), y(Value(vec.y)), z(Value(vec.z)), w(Value(vec.w)) {}
  ...
  MSK_XPU Value min_coeff() const noexcept { return std::min({x, y, z, w}); }
  ...
  MSK_XPU bool operator==(const Self &c) const { return x == c.x && y == c.y && z == c.z && w == c.w; }
  MSK_XPU bool operator!=(const Self &c) const { return x != c.x || y != c.y || z != c.z || w != c.w; }
...
// Unary operation
template <typename Value>
MSK_XPU TVector4<Value> operator-(const TVector4<Value> &vec) noexcept {
  return TVector4<Value>(-vec.x, -vec.y, -vec.z, -vec.w);
}

template <typename Value>
MSK_XPU inline auto norm(const TVector4<Value> &v) {
  return v.norm();
}

template <typename Value>
MSK_XPU inline auto squared_norm(const TVector4<Value> &v) {
  return v.squared_norm();
}

template <typename Value>
MSK_XPU inline auto normalize(const TVector4<Value> &v) {
  return v.normalize();
}

template <typename Value>
MSK_XPU inline auto abs(const TVector4<Value> &v) {
  return v.abs();
}

template <typename Value>
MSK_XPU inline auto ceil(const TVector4<Value> &v) {
  return v.ceil();
}

template <typename Value>
MSK_XPU inline auto floor(const TVector4<Value> &v) {
  return v.floor();
}

template <typename Value>
MSK_XPU inline auto hmin(const TVector4<Value> &v) {
  return v.min_coeff();
}

template <typename Value>
MSK_XPU inline auto hmax(const TVector4<Value> &v) {
  return v.max_coeff();
}

// Binary operation
template <typename Value>
MSK_XPU TVector4<Value> operator+(const TVector4<Value> &lhs, const TVector4<Value> &rhs) noexcept {
...
MSK_XPU TVector4<Value> operator+(const TVector4<Value> &lhs, T rhs) noexcept {
...
template <typename Value, typename T, std::enable_if_t<std::is_arithmetic_v<T>, int> = 0>
MSK_XPU TVector4<Value> operator+(T lhs, const TVector4<Value> &rhs) noexcept {
...
// Math functions
template <typename Value>
MSK_XPU auto dot(const TVector4<Value> &lhs, const TVector4<Value> &rhs) noexcept {
  return lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z + lhs.w * rhs.w;
}

template <typename Value>
MSK_XPU TVector4<Value> min(const TVector4<Value> &lhs, const TVector4<Value> &rhs) noexcept {
  return TVector4<Value>(std::min(lhs.x, rhs.x), std::min(lhs.y, rhs.y), std::min(lhs.z, rhs.z), std::min(lhs.w, rhs.w));
}

template <typename Value>
MSK_XPU TVector4<Value> max(const TVector4<Value> &lhs, const TVector4<Value> &rhs) noexcept {
